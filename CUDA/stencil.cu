...
void sevenPointStencil_tiledSliding_read(
        float * start,
        float * swap_out,
        const unsigned nx,
        const unsigned ny,
        ...
        )
{
    const int T = 32;
    const int Tu = 30;
    const int dimx = (nz + (Tu-1))/Tu;
    const int dimy = (ny + (Tu-1))/Tu;
    dim3 block(T,T,1);
    dim3 grid(dimx, dimy, 1);

    for (int i = 0; i < iterations; ++i){
        if(i & 1){
            sevenPointStencil_single_iter_tiled_sliding_read<T> <<< grid,block >>>(swap_out, start, nx, ny, nz);
        }
        else {
            sevenPointStencil_single_iter_tiled_sliding_read<T> <<< grid,block >>>(start, swap_out, nx, ny, nz);
        }
    }
    // we don't actually care to write the result to swap_out (in case the number of iterations is even)
...
int main()
{
    ...
    const unsigned ny = 5000;
    const unsigned nz = 5000;
...
//    CUDASSERT(cudaMemset(gpu_array2d, et_godt_primtal, x*y*sizeof(float)));
    ...
    {
        cout << "## Benchmark GPU tiled sliding read ##" << endl;

        gettimeofday(&t_startpar, NULL);

        for(unsigned x = 0; x < RUNS; x++){
            sevenPointStencil_tiledSliding_read(gpu_array3d, gpu_array3d_2, nx, ny, nz, iterations);
        }
        CUDASSERT(cudaDeviceSynchronize());

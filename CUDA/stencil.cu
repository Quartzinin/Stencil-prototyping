...
{
    const int working_block = block-2*W;
    const int grid = (2*W + len + (working_block-1)) / working_block;

    inSharedtiled_generic1d<W,block><<<grid,block>>>(start, out, len);
    CUDASSERT(cudaDeviceSynchronize());
}
